...
- **Never let base64-encoded blobs reach the assistant's output.** Tests in this repo inline modules via `data:text/javascript;base64,…` URLs; when a test fails inside an async function, stack traces include the entire encoded source — kilobytes of opaque string per frame. That output trips the API Usage Policy classifier and kills the session. Has happened multiple times. When running tests that may surface those traces, filter the output: `node --test … 2>&1 | grep -v 'data:text/javascript' | head -N`. If a test failure can't be diagnosed without the trace, fix it from the error message alone or ask the user to paste the relevant excerpt — do not pipe the raw trace into your context.
